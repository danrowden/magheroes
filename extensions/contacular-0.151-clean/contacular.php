...
class ContacularForm
{
	...
	*/
	public function processResponse($post = null, $files = null)
	{
		...
		foreach ($this->fields as $field)
		{
			...
			elseif ($field['type']=="mandatorytext" || $field['type']=="mandatorytextarea")
			{
			...
			{
				$fileExtension = $this->getFileExtension($files[$field['name']]['name']);
				if (in_array($fileExtension, $this->disallowedTypes))
				{
					$this->addError("The file extension '".$fileExtension."' is not allowed.");
				}
		...
		$subject = "Response from ".$contacularName." Form at ".$_SERVER['SERVER_NAME'];
	...
	*/
	...
	private function sanityCheck()
	{
		...
		if (!$this->recipients) return $contacularName." error: No recipient(s) were defined for this form.";
	}
